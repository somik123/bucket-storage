package org.somik.bucket.security;
...
public class WebSecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http.cors(cors -> cors.disable()).csrf(csrf -> csrf.disable())
                .authorizeHttpRequests((requests) -> requests
                        .requestMatchers("/", "/error.css", "/bucket/**", "/upload/**", "/download/**").permitAll()
    ...
    public UserDetailsService userDetailsService() {
        ...
        String bucketUser = System.getenv("BUCKET_USER");
        if (bucketUser == null || bucketUser.isEmpty())
            bucketUser = "user";
        String bucketPass = System.getenv("BUCKET_PASS");
        ...
        String hidePass = System.getenv("BUCKET_PASS_HIDE");
